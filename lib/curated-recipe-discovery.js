...
// URLs and tags rank leads only. Fresh page JSON-LD supplies every recipe fact.
const {
  isPublicRecipeUrl,
  normalizeWords,
  recipeFitsEquipment,
...
const PRODUCTION_MODES = new Set(["publisher-directions-with-link-credit", "licensed-publisher-directions-with-attribution"]);
const ALLOWED_LEAD_TAGS = new Set([
  ...
  "vegetarian", "vegan", "dairy-free", "gluten-free", "quick",
]);
...
function validateCuratedIndex(index = DEFAULT_INDEX) {
  ...
  for (const [position, policy] of policies.entries()) {
    ...
    const productionMode = String(policy?.productionMode || (String(policy?.rightsStatus || "").startsWith("prototype-only") ? "prototype-only" : ""));
    if (productionMode !== "prototype-only" && !PRODUCTION_MODES.has(productionMode)) {
      errors.push({ status: "unsupported-production-mode", sourceId: id || null });
    }
    if (productionMode === "publisher-directions-with-link-credit" && policy?.linkAttributionRequired !== true) {
      errors.push({ status: "publisher-directions-require-link-credit", sourceId: id || null });
    }
    if (productionMode === "licensed-publisher-directions-with-attribution" &&
        (policy?.attributionRequired !== true || policy?.licenseRequired !== true)) {
      errors.push({ status: "licensed-directions-require-attribution-and-license", sourceId: id || null });
    }
...
function requestSignals(input) {
  ...
  if (Number(input.maxTimeMin) > 0 && Number(input.maxTimeMin) <= 30) signals.add("quick");
  return signals;
}
...
function createCuratedRecipeDiscovery(options = {}) {
  ...
  const verifyUrlRaw = options.liveRecipeService?.verifyUrl;
  if (typeof verifyUrlRaw !== "function") throw new TypeError("A live recipe verifier with verifyUrl() is required.");
  ...
  async function verifyUrl(value) {
    let requestedUrl;
    try { requestedUrl = new URL(String(value || "")); } catch { return { ok: false, failure: { status: "unsafe-url", message: "Recipe URL is outside the curated publisher scope." } }; }
    const policy = [...policyById.values()].find((entry) => exactLeadUrl(requestedUrl.href, entry));
    const productionMode = String(policy?.productionMode || (String(policy?.rightsStatus || "").startsWith("prototype-only") ? "prototype-only" : ""));
    if (!policy || !PRODUCTION_MODES.has(productionMode)) {
      return { ok: false, failure: { status: "prototype-only-source", message: "This recipe source is not enabled for production planning." } };
    }
    if (hostBlocked.has(policy.host)) return { ok: false, failure: { status: "publisher-blocked", message: "This publisher is blocked for this request." } };
    const allowedHosts = [policy.host];
    let pageFetches = 0;
    let capHit = false;
    const result = await verifyUrlRaw(value, {
      fresh: true,
      allowedHosts,
      beforeFetch: async (currentUrl) => {
        if (pageFetches >= 4) {
          capHit = true;
          const error = new Error("Retained recipe verification exceeded its redirect fetch cap.");
          error.code = "curated-page-fetch-cap";
          throw error;
        }
        await pace(new URL(currentUrl).hostname.toLowerCase(), policy);
        pageFetches++;
      },
    });
    if (!result?.ok || !result.recipe) {
      const status = capHit ? "page-fetch-cap" : String(result?.failure?.status || "verify-failed");
      if (["403", "429"].includes(status)) hostBlocked.add(policy.host);
      return { ok: false, failure: result?.failure || { status, message: "Recipe source verification failed." } };
    }
    const recipe = result.recipe;
    const finalUrl = exactLeadUrl(recipe.finalUrl || recipe.sourceUrl || value, policy);
    if (!finalUrl) return { ok: false, failure: { status: "unsafe-redirect", message: "Recipe redirect left the publisher path." } };
    if (policy.attributionRequired && !String(recipe.attribution || "").trim()) {
      return { ok: false, failure: { status: "missing-attribution", message: "Recipe page is missing its required attribution notice." } };
    }
    if (policy.licenseRequired && !String(recipe.license || "").trim()) {
      return { ok: false, failure: { status: "missing-license", message: "Recipe page is missing its required license." } };
    }
    const rawInstructions = Array.isArray(recipe.rawInstructions) ? recipe.rawInstructions : recipe.instructions;
    if (!Array.isArray(rawInstructions) || !rawInstructions.some((step) => String(step || "").trim())) {
      return { ok: false, failure: { status: "missing-publisher-directions", message: "The verified recipe page does not provide publisher directions." } };
    }
    const linkAttribution = [String(recipe.publisher || "").trim(), String(recipe.title || "").trim(), finalUrl.href]
      .filter(Boolean).join(" | ");
    if (policy.linkAttributionRequired && (!String(recipe.publisher || "").trim() || !String(recipe.title || "").trim() || !finalUrl.href)) {
      return { ok: false, failure: { status: "missing-link-attribution", message: "Recipe page is missing publisher credit." } };
    }
    const productionEligible = productionMode === "publisher-directions-with-link-credit"
      ? !!linkAttribution
      : !!String(recipe.license || "").trim() && !!String(recipe.attribution || "").trim();
    if (!productionEligible) return { ok: false, failure: { status: "source-rights-incomplete", message: "Recipe source lacks the required credit or license metadata." } };
    return {
      ok: true,
      recipe: {
        ...recipe,
        ...
        leadUrl: String(value),
        sourcePolicyId: policy.id,
        sourceRightsStatus: policy.rightsStatus,
        linkAttribution,
        usageMode: productionMode,
  ...
  async function findRecipes(input = {}) {
    ...
    const rejected = { prototypeOnly: 0, publisherBlocked: 0, pageFetch: 0, pageParse: 0, method: 0, time: 0, equipment: 0, diet: 0, excluded: 0, attribution: 0, license: 0, duplicate: 0 };
    ...
    for (const lead of ranked) {
      ...
      if (!policy) continue;
      const productionMode = String(policy.productionMode || (String(policy.rightsStatus).startsWith("prototype-only") ? "prototype-only" : ""));
      const prototypeOnly = productionMode === "prototype-only";
      ...
      if (prototypeOnly && input.allowPrototypeOnly !== true) {
      ...
      pageChecks++;
      let capHit = false;
      const result = await verifyUrlRaw(lead.url, {
        fresh: true,
      ...
      verifiedPages++;
      const checkedRecipe = result.recipe;
      const finalUrl = exactLeadUrl(checkedRecipe.finalUrl || checkedRecipe.sourceUrl || lead.url, policy);
      if (!finalUrl) {
        rejected.pageParse++;
        rejectionReasons.push({ url: lead.url, status: "unsafe-redirect" });
        continue;
      }
      const recipe = { ...checkedRecipe, sourceUrl: finalUrl.href, finalUrl: finalUrl.href };
      ...
      const linkAttribution = [String(recipe.publisher || "").trim(), String(recipe.title || "").trim(), String(recipe.finalUrl || recipe.sourceUrl || lead.url).trim()]
        .filter(Boolean).join(" | ");
      const hasLinkCredit = !!String(recipe.publisher || "").trim() && !!String(recipe.title || "").trim() && !!String(recipe.finalUrl || recipe.sourceUrl || lead.url).trim();
      if (policy.linkAttributionRequired && !hasLinkCredit) {
        rejected.attribution++;
      ...
      if (!Array.isArray(rawInstructions) || !rawInstructions.some((step) => String(step || "").trim())) {
        rejected.method++;
        rejectionReasons.push({ url: lead.url, status: "missing-publisher-directions" });
        continue;
      }

      seenTitles.add(titleKey);
      const licenseAndAttributionReady = !!String(recipe.license || "").trim() && !!String(recipe.attribution || "").trim();
      const linkCreditReady = hasLinkCredit;
      const productionEligible = productionMode === "publisher-directions-with-link-credit"
        ? linkCreditReady
        : productionMode === "licensed-publisher-directions-with-attribution"
          ? licenseAndAttributionReady
          : false;
      candidates.push({
        ...recipe,
        leadUrl: lead.url,
        ...
        usageMode: productionMode,
        productionEligible,
        prototypeOnly: prototypeOnly || !productionEligible,
      });
    }
  ...
  const productionEligibleSourceIds = new Set(index.sourcePolicies
    .filter((policy) => PRODUCTION_MODES.has(String(policy.productionMode || "")))
  ...
  return {
    ...
    indexStats: {
      ...
      productionEligibleLeadCount: leads.filter((lead) => productionEligibleSourceIds.has(lead.sourceId)).length,
      productionEligibleSourceCount: productionEligibleSourceIds.size,
    },
    validateIndex: () => validation,
  };
}
